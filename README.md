...
- pandoc, version 1.10 or higher (optional for building the spec).
  ...
  Install the following via `apt-get install pandoc`, or `cabal install pandoc`,
  if you prefer the haskell way of doing it (this may take a while to install
  though). See <http://pandoc.org/installing.html> for other methods.
